...
function markProviderDegraded(status?: number, error?: string) {
  providerHealth = {
    ...
    lastStatus: status,
    lastError: error || PROVIDER_UNAVAILABLE,
  };
}
...
async function rateLimitGate() {
  while (true) {
    ...
    if (recentCalls.length < RATE_LIMIT_PER_MIN) {
...
function providerUnavailableRes(
  endpoint: string,
  cached: { ts: number; data: unknown } | undefined,
  status?: number,
  error?: string,
) {
...
async function healthCheck(apiKey: string): Promise<ProviderHealth> {
  ...
  if (result.ok) {
    markProviderHealthy();
    const data = result.payload && typeof result.payload === "object" && "data" in result.payload
      ? result.payload.data
      : result.payload;
  ...
  try {
    ...
    if (wantsHealthCheck || endpoint === "health") {
      ...
      return jsonRes({
        ...
        fallback: !health.ok,
        error: health.ok ? null : PROVIDER_UNAVAILABLE,
        lastCheckedAt: health.lastCheckedAt,
        upstreamStatus: health.lastStatus,
      });
    }
    ...
    if (!result.ok) {
      ...
      return providerUnavailableRes(
        endpoint,
        cached,
        result.status,
        result.payload?.message || result.payload?.msg || `Provider ${result.status}`,
      );
    }

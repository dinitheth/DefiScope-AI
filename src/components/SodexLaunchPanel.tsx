...
export default function SodexLaunchPanel({
  ...
  // No hardcoded price fallbacks — use only live Binance prices from fetchCoinList().
  // If coinPrices hasn't loaded yet, the trade setup diagram simply won't render
  // (hasTradeSetup will be false because entry === 0).
  ...
  const finalConfidence = isMainAsset ? confidence : undefined; // Don't show a fabricated confidence for non-analyzed assets

  // Signals: Only show AI-derived signals for the main (analyzed) asset.
  // For non-main assets, signals are NOT available — the user must run a briefing.
  const finalSignals = isMainAsset ? (signals ?? { momentum: "neutral", institutional: "neutral", sentiment: "neutral" }) : undefined;

  // Trade setup: Use AI-derived setup for main asset, or estimate from live Binance price for non-main.
  // isEstimatedSetup flag tracks whether the trade setup was generated by the AI or estimated locally.
  const isEstimatedSetup = !(isMainAsset && tradeSetup);
  const finalTradeSetup = isMainAsset && tradeSetup ? tradeSetup : (() => {
    const entry = coinPrices[activeAsset.toUpperCase()] ?? 0; // 0 means price not loaded yet
    if (entry === 0) return { entry: 0, target: 0, stopLoss: 0, riskReward: 0 };
      ...
      <div className="px-6 pt-6 pb-6 space-y-4">
        ...
        >
          <div className="flex items-center justify-between">
            ...
            {finalSignals && finalConfidence != null ? (
              <div
          ...
          {finalSignals ? (
            ...
            <div className="p-4 rounded-xl border text-center" style={{ background: C.surface, borderColor: `${C.warning}30` }}>
              <p className="text-[11px] font-semibold" style={{ color: C.warning }}>⚠ No AI Signals Available</p>
              <p className="text-[10px] mt-1" style={{ color: C.textMuted }}>
                Run a briefing for <span className="font-bold text-white">{activeAsset}</span> in the chat to get AI-verified momentum, institutional, and sentiment signals.
              </p>
            </div>
          )}
          ...
          <div className="flex items-center justify-between pt-3.5 border-t" style={{ borderColor: C.border }}>
            ...
            >
              {isMainAsset ? "✓ AI Verified · SoSoValue + Binance + ETF Flows" : "⚠ Live Binance Price Only"}
            </span>
          </div>

          <p className="text-[10px] leading-relaxed" style={{ color: C.textMuted }}>
            {isMainAsset
              ? "All signals and trade levels generated by the AI Decision Engine using live Binance spot prices, SoSoValue ETF flow data, and news sentiment analysis."
              : `Only the live Binance spot price is available for ${activeAsset}. Ask the AI to analyze ${activeAsset} in the chat to get verified signals, trade setup, and confidence score.`}
          </p>
        </motion.div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                ...
                <p className="text-[10px] uppercase tracking-wider font-extrabold" style={{ color: finalSide === "BUY" ? C.success : C.danger }}>
                  {isEstimatedSetup ? `${finalSide} Trade Estimate` : `AI ${finalSide} Trade Setup`}
                </p>
              </div>
              <div className="flex items-center gap-2">
